...
def initialize_event_with_slot(event_slot_number, event_id, *event_args):
    ...
    return _format_event(event_format, event_slot_number, event_id, *event_args)

# 2
def set_network_sync(network_sync_state):
    event_format = ['2000', '02', 'B']
    network_sync_state = 1 if network_sync_state else 0
    return _format_event(event_format, network_sync_state)

# 4
def issue_prefetch_request(request_id):
    event_format = ['2000', '04', 'I']
    return _format_event(event_format, request_id)
...
def save_request():
    ...
    index = '05'
    arg_format = 'B'
    return _format_event(group, index, arg_format, 0)
...
def play_cutscene_and_warp_player(cutscene_id, playback_method, point_entity_id, area_id, block_id):
    ...
    return _format_event(event_format, cutscene_id, playback_method, point_entity_id, area_id, block_id)
    
# 3
def play_cutscene_to_player(cutscene_id, playback_method, player_entity_id):
    event_format = ['2002', '03', 'iIi']
    return _format_event(event_format, cutscene_id, playback_method, player_entity_id)
    
# 4
def play_cutscene_and_warp_specific_player(cutscene_id, playback_method, point_entity_id, area_id, block_id, player_entity_id):
    event_format = ['2002', '04', 'iIiBBi']
    return _format_event(event_format, cutscene_id, playback_method, point_entity_id, area_id, block_id, player_entity_id)
    
# 5
def play_cutscene_and_rotate_player(cutscene_id, playback_method, axis_x, axis_z, rotation, translation_y, player_entity_id):
    event_format = ['2002', '05', 'iIffifi']
    return _format_event(event_format, cutscene_id, playback_method, axis_x, axis_z, rotation, translation_y, player_entity_id)
...
def animation_playback_request(entity_id, animation_id, loop, wait_for_completion):
    ...
    wait_for_completion = 1 if wait_for_completion else 0
    return _format_event(event_format, entity_id, animation_id, loop, wait_for_completion)
...
def set_event_flag(event_flag_id, desired_state):
    ...
    desired_state = 1 if desired_state else 0
    return _format_event(event_format, event_flag_id, desired_state)
...
def set_spawner_state(entity_id, desired_state):
    ...
    desired_state = 1 if desired_state else 0
    return _format_event(event_format, entity_id, desired_state)
    
# 4
def award_item_lot(item_lot_id):
    # Directly gives to player (pops up on screen).
    event_format = ['2003', '04', 'i']
    return _format_event(event_format, item_lot_id)
    
# 5
def shoot_projectile(owner_entity_id, projectile_entity_id, damipoly_id, behavior_id, launch_angle_x, launch_angle_y, launch_angle_z):
    event_format = ['2003', '05', 'iiiiiii']
    return _format_event(event_format, owner_entity_id, projectile_entity_id, damipoly_id, behavior_id, launch_angle_x, launch_angle_y, launch_angle_z)
...
def set_event_id_state_with_slot(event_id, event_slot_id, desired_state):
    ...
    desired_state = 1 if desired_state else 0
    return _format_event(event_format, event_id, event_slot_id, desired_state)
...
def set_boss_health_bar_with_slot(desired_state, entity_id, slot_number, name_id):
    ...
    return _format_event(event_format, desired_state, entity_id, slot_number, name_id)

# 12
def kill_boss(entity_id):
    event_format = ['2003', '12', 'i']
    return _format_event(event_format, entity_id)
...
def modify_navmesh_collision_bitflags(entity_id, navimesh_collision_bit, modification_type):
    ...
    return _format_event(event_format, entity_id, navimesh_collision_bit, modification_type)

# 14
def warp_player(area_id, block_id, area_entity_id):
    event_format = ['2003', '14', 'BBi']
    return _format_event(event_format, area_id, block_id, area_entity_id)

# 16
def trigger_multiplayer_event(multiplayer_event_id):
    event_format = ['2003', '16', 'i']
    return _format_event(event_format, multiplayer_event_id)
...
def randomly_set_one_flag_in_range(start_event_flag_id, end_event_flag_id, desired_state):
    ...
    desired_state = 1 if desired_state else 0
    return _format_event(event_format, start_event_flag_id, end_event_flag_id, desired_state)
...
def force_animation(entity_id, animation_id, loop, wait_for_completion, do_not_wait_for_transition):
    ...
    loop = 1 if loop else 0
    wait_for_completion = 1 if wait_for_completion else 0
    do_not_wait_for_transition = 1 if do_not_wait_for_transition else 0
    return _format_event(event_format, entity_id, animation_id, loop, wait_for_completion, do_not_wait_for_transition)

# 19
def set_area_texture_parambank_slot_index(area_id, texture_parambank_slot_index):
    event_format = ['2003', '19', 'hh']
    return _format_event(event_format, area_id, texture_parambank_slot_index)

# 21
def increment_ngplus_counter():
    event_format = ['2003', '21', 'B']
    return _format_event(event_format, 0)
...
def place_NPC_summon_sign(sign_type, entity_id, summon_point, summon_event_flag_id, dismissal_event_flag_id):
    ...
    return _format_event(event_format, sign_type, entity_id, summon_point, summon_event_flag_id, dismissal_event_flag_id)
...
def activate_player_killplane(map_id, block_id, threshold_Y, target_model_id):
    ...
    return _format_event(event_format, map_id, block_id, threshold_Y, target_model_id)
...
def switch_allegiance(entity_id, new_team):
    ...
    return _format_event(event_format, entity_id, new_team)
...
def warp(entity_id, warp_destination_type, destination_target_id, damipoly_id):
    ...
    return _format_event(event_format, entity_id, warp_destination_type, destination_target_id, damipoly_id)

# 4
def kill(entity_id, yields_souls=0):
    # Technically a kill 'request.'
    event_format = ['2004', '04', 'iB']
    yields_souls = 1 if yields_souls else 0
    return _format_event(event_format, entity_id, yields_souls)
...
def ezstate_instruction_request(entity_id, command_id, slot_number):
    ...
    return _format_event(event_format, entity_id, command_id, slot_number)
...
def set_special_effect(entity_id, special_effect_id):
    ...
    return _format_event(event_format, entity_id, special_effect_id)

# 9
def set_standby_animation_settings_to_default(entity_id):
    return set_standby_animation_settings(entity_id, -1, -1, -1, -1, -1)
def set_standby_animation_settings(entity_id, standby_animation, damage_animation, cancel_animation, death_animation, standby_return_animation):
    # Sets entity's standby animations. -1 is default for each category.
    event_format = ['2004', '09', 'iiiiii']
    return _format_event(event_format, entity_id, standby_animation, damage_animation, cancel_animation, death_animation, standby_return_animation)
...
def set_event_point(entity_id, event_area_entity_id, reaction_range):
    ...
    return _format_event(event_format, entity_id, event_area_entity_id, reaction_range)

# 19
def set_ai_id(entity_id, ai_id):
    event_format = ['2004', '19', 'ii']
    return _format_event(event_format, entity_id, ai_id)
...
def create_multipart_NPC_part(entity_id, part_npc_type, part_index, part_health,
    ...
    is_invincible = 1 if is_invincible else 0
    start_in_stop_state = 1 if start_in_stop_state else 0
    return _format_event(event_format, entity_id, part_npc_type, part_index, part_health,
                              damage_correction, body_damage_correction, is_invincible, start_in_stop_state)
    
# 23
def set_multipart_NPC_part_health(entity_id, part_npc_type, desired_hp, overwrite_max):
    event_format = ['2004', '23', 'iiiB']
    overwrite_max = 1 if overwrite_max else 0
    return _format_event(event_format, entity_id, part_npc_type, desired_hp, overwrite_max)
...
def set_multipart_NPC_part_effects(entity_id, part_npc_type, material_special_effect_id, material_SFX_id):
    ...
    return _format_event(event_format, entity_id, part_npc_type, material_special_effect_id, material_SFX_id)

# 25
def set_multipart_NPC_part_bullet_damage_scaling(entity_id, part_npc_type, desired_scaling):
    event_format = ['2004', '25', 'iif']
    return _format_event(event_format, entity_id, part_npc_type, desired_scaling)

# 26
def set_display_mask(entity_id, bit_number, switch_type):
    # 0 = off, 1 = on, 2 = change
    event_format = ['2004', '26', 'iBB']
    return _format_event(event_format, entity_id, bit_number, switch_type)
...
def set_network_update_authority(entity_id, authority_level):
    ...
    return _format_event(event_format, entity_id, authority_level)
...
def set_collision(entity_id, disable_collision):
    ...
    disable_collision = 1 if disable_collision else 0
    return _format_event(event_format, entity_id, disable_collision)
...
def ai_event(entity_id, command_id, slot_number, start_event_flag_id, end_event_flag_id):
    ...
    return _format_event(event_format, entity_id, command_id, slot_number, start_event_flag_id, end_event_flag_id)
...
def set_network_update_rate(entity_id, is_fixed, frequency):
    ...
    return _format_event(event_format, entity_id, is_fixed, frequency)
...
def hellkite_breath_control(entity_id, object_entity_id, animation_id):
    ...
    return _format_event(event_format, entity_id, object_entity_id, animation_id)
...
def warp_and_set_floor(entity_id, warp_destination_type, damipoly_id, destination_entity_id):
    ...
    return _format_event(event_format, entity_id, warp_destination_type, damipoly_id, destination_entity_id)
...
def warp_and_copy_floor(entity_id, warp_destination_type, destination_target_id, damipoly_id, copy_floor_of_entity_id):
    ...
    return _format_event(event_format, entity_id, warp_destination_type, destination_target_id, damipoly_id, copy_floor_of_entity_id)

# 43
def reset_animation(entity_id, disable_interpolation):
    # 0 = interpolated, 1 = not interpolated
    event_format = ['2004', '43', 'iB']
    disable_interpolation = 1 if disable_interpolation else 0
    return _format_event(event_format, entity_id, disable_interpolation)
...
def NPC_humanity_registration(entity_id, event_flag_id):
    ...
    return _format_event(event_format, entity_id, event_flag_id)
...
def destroy_object(entity_id, slot_number):
    ...
    return _format_event(event_format, entity_id, slot_number)
...
def set_object_state(entity_id, activation_status):
    ...
    return _format_event(event_format, entity_id, activation_status)
...
def start_object_activation(entity_id, object_parameter_id, relative_idx):
    ...
    return _format_event(event_format, entity_id, object_parameter_id, relative_idx)
...
def set_object_activation(entity_id, object_parameter_id, activation_status):
    ...
    return _format_event(event_format, entity_id, object_parameter_id, activation_status)
...
def skip_to_end_of_animation(entity_id, animation_id):
    ...
    return _format_event(event_format, entity_id, animation_id)
...
# 9
def create_damaging_object(entity_flag_id, entity_id, damipoly_id, behavior_id, target_type, radius, life, repetition_time):
    ...
    return _format_event(event_format, entity_flag_id, entity_id, damipoly_id, behavior_id, target_type, radius, life, repetition_time)
...
def register_statue_object(entity_id, area_number, block_number, statue_type):
    ...
    return _format_event(event_format, entity_id, area_number, block_number, statue_type)
...
def warp_object_to_character(entity_id, character_entity_id, damipoly_id):
    ...
    return _format_event(event_format, entity_id, character_entity_id, damipoly_id)

# 12
def remove_object_event_flag(event_flag_id):
    # TODO: Check then this is actually used.
...
def set_object_invulnerability(entity_id, invulnerability_state):
    ...
    return _format_event(event_format, entity_id, invulnerability_state)
...
def set_object_activation_with_idx(entity_id, object_parameter_id, relative_idx, activation_state):
    ...
    return _format_event(event_format, entity_id, object_parameter_id, relative_idx, activation_state)
...
# 1
def delete_map_sfx(entity_id, erase_root_only=1):
    # Erasing the root only probably allows easy recreation later (default).
    event_format = ['2006', '01', 'iB']
    return _format_event(event_format, entity_id, erase_root_only)
...
def create_oneoff_sfx(sfx_type, entity_id, damipoly_id, sfx_id):
    ...
    return _format_event(event_format, sfx_type, entity_id, damipoly_id, sfx_id)

# 4
def create_object_sfx(entity_id, damipoly_id, sfx_id):
    event_format = ['2006', '04', 'iii']
    return _format_event(event_format, entity_id, damipoly_id, sfx_id)

# 5
def delete_object_sfx(entity_id, erase_root=1):
    # Note `erase_root` vs. `erase_root_only` for map SFX.
    event_format = ['2006', '05', 'ii']
    return _format_event(event_format, entity_id, erase_root)
...
def display_generic_dialog(message_id, button_type, number_buttons, entity_id, display_distance):
    ...
    return _format_event(event_format, message_id, button_type, number_buttons, entity_id, display_distance)
...
def display_text_banner(banner_type):
    ...
    return _format_event(event_format, banner_type)
...
def display_status_explanation_message(message_id, pad_enabled=0):
    ...
    return _format_event(event_format, message_id, pad_enabled)

# 4
def display_battlefield_message(message_id, display_location_index):
    # TODO: Check usage.
    event_format = ['2007', '04', 'iB']
    return _format_event(event_format, message_id, display_location_index)
...
def set_locked_camera_slot_number(area_id, block_id, locked_camera_slot_number):
    ...
    return _format_event(event_format, area_id, block_id, locked_camera_slot_number)
...
def register_ladder(event_flag_id_1, event_flag_id_2, entity_id):
    ...
    return _format_event(event_format, event_flag_id_1, event_flag_id_2, entity_id)
...
def register_bonfire(event_flag_id, entity_id, reaction_distance, reaction_angle, initial_basic_spot_point):
    ...
    return _format_event(event_format, event_flag_id, entity_id, reaction_distance, reaction_angle, initial_basic_spot_point)
...
def play_sound_effect(entity_id, sound_type, sound_id):
    ...
    return _format_event(event_format, entity_id, sound_type, sound_id)
...
def set_map_sound(entity_id, sound_state):
    ...
    return _format_event(event_format, entity_id, sound_state)
...
def set_hitbox_state(entity_id, activation_state):
    ...
    return _format_event(event_format, entity_id, activation_state)
...
def set_map_part_state(map_part_id, activation_state):
    ...
    return _format_event(event_format, map_part_id, activation_state)

""" 1000: EXECUTION CONTROL (SYSTEM) """

# 1
def skip_if_register_true(number_lines, register=0):
    # Skips some number of lines if the register is true.
    # Default register is MAIN.
    return skip_if_register_state(number_lines, 1, register)
def skip_if_register_false(number_lines, register=0):
    # Skips some number of lines if the register is false.
    # Default register is MAIN.
    return skip_if_register_state(number_lines, 0, register)
def skip_if_register_state(number_lines, required_state, register):
    # Skips some number of lines if the specified register has the specified
    # state.
    """
    Register list:
        -7: "OR(07)", 
    ...
    return _format_event(event_format, number_lines, required_state, register)

# 2
def restart_if_register_true(register=0):
    # Restart the event if the register is true.
    # Default register is MAIN.
    return terminate_if_register_state(1, 1, register)
def restart_if_register_false(register=0):
    # Restart the event if the register is false.
    # Default register is MAIN.
    return terminate_if_register_state(1, 0, register)
def end_if_register_true(register=0):
    # End (not restart) the event if the register is true.
    # Default register is MAIN.
    return terminate_if_register_state(0, 1, register)
def end_if_register_false(register=0):
    # End (not restart) the event if the register is true.
    # Default register is MAIN.
    return terminate_if_register_state(0, 0, register)
def terminate_if_register_state(event_end_type, required_state, register):
    event_format = ['1000', '02', 'BBb']
    return _format_event(event_format, event_end_type, required_state, register)

# 3
def skip(number_lines):
    # Unconditional line skip.
    event_format = ['1000', '03', 'B']
    return _format_event(event_format, number_lines)
...
def terminate(event_end_type):
    ...
    return _format_event(event_format, event_end_type)
...
def skip_if_value_comparison(number_lines, comparison_type, left, right):
    ...
    return _format_event(event_format, number_lines, comparison_type, left, right)
...
def terminate_if_value_comparison(event_end_type, comparison_type, left, right):
    ...
    return _format_event(event_format, event_end_type, comparison_type, left, right)

# 7
def skip_if_register_true_finished(number_lines, register=0):
    # Default register is MAIN.
    return skip_if_register_state_finished(number_lines, 1, register)
def skip_if_register_false_finished(number_lines, register=0):
    # Default register is MAIN.
    return skip_if_register_state_finished(number_lines, 0, register)
def skip_if_register_state_finished(number_lines, required_state, register):
    # It is unclear how this differs from 1000[01]. The instruction name says
    # "finished condition group" (register) rather than simply "condition
    # group". This may use the register in a slightly different way.
    # TODO: Examine when this is used versus 1000[01].
    event_format = ['1000', '07', 'BBb']
    return _format_event(event_format, number_lines, required_state, register)
    
# 8
def restart_if_register_true_finished(register=0):
    # Restart the event if the register is true.
    # Default register is MAIN.
    return terminate_if_register_state_finished(1, 1, register)
def restart_if_register_false_finished(register=0):
    # Restart the event if the register is false.
    # Default register is MAIN.
    return terminate_if_register_state_finished(1, 0, register)
def end_if_register_true_finished(register=0):
    # End (not restart) the event if the register is true.
    # Default register is MAIN.
    return terminate_if_register_state_finished(0, 1, register)
def end_if_register_false_finished(register=0):
    # End (not restart) the event if the register is true.
    # Default register is MAIN.
    return terminate_if_register_state_finished(0, 0, register)
def terminate_if_register_state_finished(event_end_type, required_state, register):
...
def wait_for_network_approval(timeout):
    ...
    return _format_event(event_format, timeout)
...
def wait(number_seconds):
    ...
    return _format_event(event_format, number_seconds)

# 1
def wait_frames(number_frames):
    # Wait for some number of frames.
    event_format = ['1001', '01', 'i']
    return _format_event(event_format, number_frames)
...
def wait_random_range(min_number_seconds, max_number_seconds):
    ...
    return _format_event(event_format, min_number_seconds, max_number_seconds)
...
def skip_if_event_flag_on(number_lines, event_flag_type, event_flag_id):
    return skip_if_event_flag_state(number_lines, 1, event_flag_type, event_flag_id)
def skip_if_event_flag_off(number_lines, event_flag_type, event_flag_id):
    return skip_if_event_flag_state(number_lines, 0, event_flag_type, event_flag_id)
def skip_if_event_flag_state(number_lines, required_flag_state, event_flag_type, event_flag_id):
    ...
    return _format_event(event_format, number_lines, required_flag_state, event_flag_type, event_flag_id)
...
def end_if_event_flag_on(event_flag_type, event_flag_id):
    return terminate_if_event_flag_state(0, 1, event_flag_type, event_flag_id)
def end_if_event_flag_off(event_flag_type, event_flag_id):
    return terminate_if_event_flag_state(0, 0, event_flag_type, event_flag_id)
def restart_if_event_flag_on(event_flag_type, event_flag_id):
    return terminate_if_event_flag_state(1, 1, event_flag_type, event_flag_id)
def restart_if_event_flag_off(event_flag_type, event_flag_id):
    return terminate_if_event_flag_state(1, 0, event_flag_type, event_flag_id)
def terminate_if_event_flag_state(event_end_type, required_flag_state, event_flag_type, event_flag_id):
    # Terminate (end or restart) event if the specified flag has the specified
    # state (0 = off, 1 = on).
    event_format = ['1003', '02', 'BBBi']
    return _format_event(event_format, event_end_type, required_flag_state, event_flag_type, event_flag_id)

# 3
def skip_if_event_flag_range_on(number_lines, event_flag_type, start_event_flag_id, end_event_flag_id):
    return skip_if_event_flag_range_state(number_lines, 1, event_flag_type, start_event_flag_id, end_event_flag_id)
def skip_if_event_flag_range_off(number_lines, event_flag_type, start_event_flag_id, end_event_flag_id):
    return skip_if_event_flag_range_state(number_lines, 0, event_flag_type, start_event_flag_id, end_event_flag_id)
def skip_if_event_flag_range_state(number_lines, required_flag_state, event_flag_type, start_event_flag_id, end_event_flag_id):
    ...
    return _format_event(event_format, number_lines, required_flag_state, event_flag_type, start_event_flag_id, end_event_flag_id)

# 4
def end_if_event_flag_range_on(event_flag_type, start_event_flag_id, end_event_flag_id):
    return terminate_if_event_flag_range_state(0, 1, event_flag_type, start_event_flag_id, end_event_flag_id)
def end_if_event_flag_range_off(event_flag_type, start_event_flag_id, end_event_flag_id):
    return terminate_if_event_flag_range_state(0, 0, event_flag_type, start_event_flag_id, end_event_flag_id)
def restart_if_event_flag_range_on(event_flag_type, start_event_flag_id, end_event_flag_id):
    return terminate_if_event_flag_range_state(1, 1, event_flag_type, start_event_flag_id, end_event_flag_id)
def restart_if_event_flag_range_off(event_flag_type, start_event_flag_id, end_event_flag_id):
    return terminate_if_event_flag_range_state(1, 0, event_flag_type, start_event_flag_id, end_event_flag_id)
def terminate_if_event_flag_range_state(event_end_type, required_flag_state, event_flag_type, start_event_flag_id, end_event_flag_id):
    # Terminate (end or restart) event if the specified range of flags all have
    # the specified state (0 = off, 1 = on).
    event_format = ['1003', '04', 'BBBii']
    return _format_event(event_format, event_end_type, required_flag_state, event_flag_type, start_event_flag_id, end_event_flag_id)
...
def skip_if_multiplayer_state(number_lines, required_multiplayer_state):
    ...
    return _format_event(event_format, number_lines, required_multiplayer_state)
...
def terminate_if_multiplayer_state(event_end_type, required_multiplayer_state):
    ...
    return _format_event(event_format, event_end_type, required_multiplayer_state)
...
def skip_if_area_state(number_lines, required_area_state, area_id, block_id):
    ...
    return _format_event(event_format, number_lines, required_area_state, area_id, block_id)
...
def skip_if_object_destruction_state(number_lines, required_destruction_state, entity_id):
    ...
    return _format_event(event_format, number_lines, required_destruction_state, entity_id)
...
def terminate_if_object_destruction_state(event_end_type, required_destruction_state, entity_id):
    ...
    return _format_event(event_format, event_end_type, required_destruction_state, entity_id)

""" 0: CONDITION REGISTERS (SYSTEM) """

# 0
def if_register_true(output_register, input_register):
    return if_register_state(output_register, 1, input_register)
def if_register_false(output_register, input_register):
    return if_register_state(output_register, 0, input_register)
def if_register_state(output_register, required_result, input_register):
    # Evaluates the input register (as OR or AND), compares it to the
    # required result, and stores the result of the comparison in the output
    # register (where many values can be stored). 
    # The required result is 1 by default, which means that the output register 
    # will simply store the evaluation of the input.
    event_format = ['   0', '00', 'bBb']
    return _format_event(event_format, output_register, required_result, input_register)

""" 1: CONDITION REGISTERS (TIME) """

# 0
def if_time_elapsed(output_register, number_seconds):
    # Counts seconds since event started (I think).
    # TODO: Confirm time since event started.
    event_format = ['   1', '00', 'bf']
    return _format_event(event_format, output_register, number_seconds)

# 1
def if_frames_elapsed(output_register, number_frames):
    # TODO: Confirm number of frames since event started.
    event_format = ['   1', '01', 'bi']
    return _format_event(event_format, output_register, number_frames)

# 2 and 3 choose a random number of seconds/frames, I think, but unused.

""" 3: CONDITION REGISTERS (EVENT) """

# 0
def if_event_flag_on(output_register, event_flag_type, event_flag_id):
    return if_event_flag_state(output_register, 1, event_flag_type, event_flag_id)
def if_event_flag_off(output_register, event_flag_type, event_flag_id):
    return if_event_flag_state(output_register, 0, event_flag_type, event_flag_id)
def if_event_flag_state(output_register, required_flag_state, event_flag_type, event_flag_id):
    event_format = ['   3', '00', 'bBBi']
    return _format_event(event_format, output_register, required_flag_state, event_flag_type, event_flag_id)

# 1
def if_event_flag_range_on(output_register, event_flag_type, start_event_flag_id, end_event_flag_id):
    return if_event_flag_state(output_register, 1, event_flag_type, start_event_flag_id, end_event_flag_id)
def if_event_flag_range_off(output_register, event_flag_type, start_event_flag_id, end_event_flag_id):
    return if_event_flag_state(output_register, 0, event_flag_type, start_event_flag_id, end_event_flag_id)
def if_event_flag_range_state(output_register, required_flag_state, event_flag_type, start_event_flag_id, end_event_flag_id):
    event_format = ['   3', '01', 'bBBii']
    return _format_event(event_format, output_register, required_flag_state, event_flag_type, start_event_flag_id, end_event_flag_id)

# 2
def if_entity_inside_area(output_register, entity_id, area_entity_id):
    return if_entity_inside_or_outside_area(output_register, entity_id, area_entity_id, 1)
def if_entity_outside_area(output_register, entity_id, area_entity_id):
    return if_entity_inside_or_outside_area(output_register, entity_id, area_entity_id, 0)
def if_player_inside_area(output_register, area_entity_id):
    return if_entity_inside_or_outside_area(output_register, 10000, area_entity_id, 1)
def if_player_outside_area(output_register, area_entity_id):
    return if_entity_inside_or_outside_area(output_register, 10000, area_entity_id, 0)
def if_entity_inside_or_outside_area(output_register, entity_id, area_entity_id, is_inside):
    # Checks if specified entity is inside or outside specified area.
    # 0 = outside, 1 = inside.
    # Note that argument order has changed.
    event_format = ['   3', '02', 'bBii']
    return _format_event(event_format, output_register, is_inside, entity_id, area_entity_id)

# 3
def if_player_within_distance(output_register, target_entity_id, required_distance):
    return if_entity_within_or_beyond_distance(output_register, 10000, target_entity_id, required_distance, 1)
def if_player_beyond_distance(output_register, target_entity_id, required_distance):
    return if_entity_within_or_beyond_distance(output_register, 10000, target_entity_id, required_distance, 0)
def if_entity_within_distance(output_register, first_entity_id, second_entity_id, required_distance):
    return if_entity_within_or_beyond_distance(output_register, first_entity_id, second_entity_id, required_distance, 1)
def if_entity_beyond_distance(output_register, first_entity_id, second_entity_id, required_distance):
    return if_entity_within_or_beyond_distance(output_register, first_entity_id, second_entity_id, required_distance, 0)
def if_entity_within_or_beyond_distance(output_register, first_entity_id, second_entity_id, required_distance, is_within):
    # Check is entity A is within (is_within = 1) or beyond the specified
    # distance of entity B.
    event_format = ['   3', '03', 'bBiif']
    return _format_event(event_format, output_register, is_within, first_entity_id, second_entity_id, required_distance)

# 4
def if_player_has_item(output_register, item_type, item_id):
    return if_player_has_or_does_not_have_item(output_register, item_type, item_id, 1)
def if_player_does_not_have_item(output_register, item_type, item_id):
    return if_player_has_or_does_not_have_item(output_register, item_type, item_id, 0)
def if_player_has_or_does_not_have_item(output_register, item_type, item_id, required_state):
    # Check if player has specified item in inventory, not including Bototmless
    # Box (required_state = 1) or does not have the item (required_state = 0).
    event_format = ['   3', '04', 'bBiB']
    return _format_event(event_format, output_register, item_type, item_id, required_state)

# 5
# TODO: Check the most common parameters for this (e.g. doors) and make a
# shortcut function for them.
def if_action_button_state(output_register, category, target_entity_id, reaction_angle, damipoly_id, reaction_distance, help_id, reaction_attribute, pad_id):
    ...
    return _format_event(event_format, output_register, category, target_entity_id, reaction_angle, damipoly_id, reaction_distance, help_id, reaction_attribute, pad_id)
    
# 6
def if_host(output_register):
    return if_multiplayer_state(output_register, 0)
def if_client(output_register):
    return if_multiplayer_state(output_register, 1)
def if_singleplayer(output_register):
    return if_multiplayer_state(output_register, 2)
def if_multiplayer(output_register):
    return if_multiplayer_state(output_register, 3)
def if_multiplayer_state(output_register, required_state):
    # Check if player is host (0), summon (1), single player (2), or
    # multiplayer (3).
    event_format = ['   3', '06', 'bb']
    return _format_event(event_format, output_register, required_state)

# 7
def if_all_players_inside_area(output_register, area_entity_id):
    return if_all_players_inside_or_outside_area(output_register, area_entity_id, 1)
def if_all_players_outside_area(output_register, area_entity_id):
    return if_all_players_inside_or_outside_area(output_register, area_entity_id, 0)
def if_all_players_inside_or_outside_area(output_register, area_entity_id, is_inside):
    # Check if all players are inside (1) or outside (0) the specified area.
    event_format = ['   3', '07', 'bBi']
    return _format_event(event_format, output_register, is_inside, area_entity_id)

# 8
def if_in_world_area(output_register, area_id, block_id):
    return if_world_area_state(output_register, area_id, block_id, 1)
def if_not_in_world_area(output_register, area_id, block_id):
    return if_world_area_state(output_register, area_id, block_id, 0)
def if_world_area_state(output_register, area_id, block_id, is_inside):
    # Check if player is inside or outside the specified world area and block.
    event_format = ['   3', '08', 'bBBB']
    return _format_event(event_format, output_register, is_inside, area_id, block_id)

# 9
def if_multiplayer_event(output_register, multiplayer_event_id):
    # Check if a multiplayer event has occured.
    event_format = ['   3', '09', 'bI']
    return _format_event(event_format, output_register, multiplayer_event_id)

# 10
def if_at_least_one_true_flag_in_range(output_register, event_flag_type, start_event_flag_id, end_event_flag_id):
    # Check if at least one flag in the specified range is true.
    return if_count_true_event_flags_in_range(output_register, event_flag_type, start_event_flag_id, end_event_flag_id, 4, 1)
def if_number_true_flags_in_range_greater_than_or_equal(output_register, event_flag_type, start_event_flag_id, end_event_flag_id, min_count):
    # Check if the number of true flags in the specified range is greater than
    # or equal to the specified minimum.
    return if_count_true_event_flags_in_range(output_register, event_flag_type, start_event_flag_id, end_event_flag_id, 4, min_count)
def if_count_true_event_flags_in_range(output_register, event_flag_type, start_event_flag_id, end_event_flag_id, comparison_type, count_comparison):
    ...
    return _format_event(event_format, output_register, event_flag_type, start_event_flag_id, end_event_flag_id, comparison_type, count_comparison)

# 11
def if_world_tendency_greater_than_or_equal(output_register, tendency_type, min_tendency):
    return if_world_tendency_greater_than_or_equal(output_register, tendency_type, 4, min_tendency)
def if_world_tendency_comparison(output_register, tendency_type, comparison_type, tendency_comparison):
    ...
    return _format_event(event_format, output_register, tendency_type, comparison_type, tendency_comparison)

# 12
def if_event_value_comparison(output_register, event_flag_id, number_bits, comparison_type, comparison_value):
    # Check if specified bit in event value (usually 0 I think) compares true
    # with specified value.
    event_format = ['   3', '12', 'biBBI']
    return _format_event(event_format, output_register, event_flag_id, number_bits, comparison_type, comparison_value)

# 13
def if_action_button_state_in_boss(output_register, category, target_entity_id, reaction_angle, damipoly_id, reaction_distance, help_id, reaction_attribute, pad_id):
...
# 14
def if_any_item_dropped_in_area(output_register, area_entity_id):
    # Check if any item has been dropped in the specified area.
    event_format = ['   3', '14', 'bi']
    return _format_event(event_format, output_register, area_entity_id)

# 15
def if_item_dropped(output_register, item_type, item_id):
    # Check if a specified item has been dropped (anywhere).
    event_format = ['   3', '15', 'bii']
    return _format_event(event_format, output_register, item_type, item_id)

# 16
def if_player_owns_item(output_register, item_type, item_id):
    ...
    return _format_event(event_format, output_register, item_type, item_id, 1)
def if_player_does_not_own_item(output_register, item_type, item_id):
    # Includes Bottomless Box.
    event_format = ['   3', '16', 'bBiB']
    return _format_event(event_format, output_register, item_type, item_id, 0)

# 17
def if_new_game_count_equal(output_register, completion_count_comparison):
    # Checks if count of completed playthroughs is equal to some value.
    return if_new_game_count_comparison(output_register, 0, completion_count_comparison)
def if_new_game_count_greater_than_or_equal(output_register, min_completion_count):
    # Checks if count of completed playthroughs is greater than or equal to some value.
    return if_new_game_count_comparison(output_register, 4, min_completion_count)
def if_new_game_count_comparison(output_register, comparison_type, completion_count_comparison):
    # Checks count of completed playthroughs and compares to value.
    event_format = ['   3', '17', 'bBB']
    return _format_event(event_format, output_register, comparison_type, completion_count_comparison)

# 18
def if_action_button_state_and_line_segment(output_register, category, target_entity_id, reaction_angle, damipoly_id, reaction_distance, help_id, reaction_attribute, pad_id, line_segment_endpoint_id):
    ...
    return _format_event(event_format, output_register, category, target_entity_id, reaction_angle, damipoly_id, reaction_distance, help_id, reaction_attribute, pad_id, line_segment_endpoint_id)

# 19
def if_action_button_state_and_line_segment_in_boss(output_register, category, target_entity_id, reaction_angle, damipoly_id, reaction_distance, help_id, reaction_attribute, pad_id, line_segment_endpoint_id):
    # Checks state of action button (A on the Xbox controller) and, I assume,
...
# 20
def if_event_flag_value_comparison(output_register, left_event_flag_id, left_number_bits, comparison_type, right_event_flag_id, right_number_bits):
    # Check comparison of two event flag values.
    event_format = ['   3', '20', 'biBBiB']
    return _format_event(event_format, output_register, left_event_flag_id, left_number_bits, comparison_type, right_event_flag_id, right_number_bits)

# 21
def if_owns_DLC(output_register):
    # Check if player owns Artorias of the Abyss DLC expansion.
    # NOTE: Again, no generic function here.
    event_format = ['   3', '21', 'bB']
    return _format_event(event_format, output_register, 1)
def if_does_not_own_DLC(output_register):
    # Check if player does not own Artorias of the Abyss DLC expansion.
    event_format = ['   3', '21', 'bB']
    return _format_event(event_format, output_register, 0)

# 22
def if_online(output_register):
    return if_online_state(output_register, 1)
def if_offline(output_register):
    return if_online_state(output_register, 0)
def if_online_state(output_register, online_state):
    # Check if player is online (1) or offline (0).
    event_format = ['   3', '22', 'bB']
    return _format_event(event_format, output_register, online_state)

""" 4: CONDITION REGISTERS (CHARACTER) """

# 0
def if_entity_dead(output_register, entity_id):
    return if_entity_death_state(output_register, entity_id, 1)
def if_entity_alive(output_register, entity_id):
    return if_entity_death_state(output_register, entity_id, 0)
def if_entity_death_state(output_register, entity_id, required_state):
    # Check if entity is alive (0) or dead (1).
    event_format = ['   4', '00', 'biB']
    return _format_event(event_format, output_register, entity_id, required_state)

# 1
def if_entity_hostile(output_register, entity_id, attacking_entity_id):
    ...
    return _format_event(event_format, output_register, entity_id, attacking_entity_id)

# 2
def if_entity_health_equal(output_register, entity_id, health_comparison):
    # Check if entity health == value.
    return if_entity_health_comparison(output_register, entity_id, 0, health_comparison)
def if_entity_health_not_equal(output_register, entity_id, health_comparison):
    # Check if entity health != value.
    return if_entity_health_comparison(output_register, entity_id, 1, health_comparison)
def if_entity_health_greater_than(output_register, entity_id, health_comparison):
    # Check if entity health > value.
    return if_entity_health_comparison(output_register, entity_id, 2, health_comparison)
def if_entity_health_less_than(output_register, entity_id, health_comparison):
    # Check if entity health < value.
    return if_entity_health_comparison(output_register, entity_id, 3, health_comparison)
def if_entity_health_greater_than_or_equal(output_register, entity_id, health_comparison):
    # Check if entity health >= value.
    return if_entity_health_comparison(output_register, entity_id, 4, health_comparison)
def if_entity_health_less_than_or_equal(output_register, entity_id, health_comparison):
    # Check if entity health <= value.
    return if_entity_health_comparison(output_register, entity_id, 5, health_comparison)
def if_entity_health_comparison(output_register, entity_id, comparison_type, health_comparison):
    ...
    return _format_event(event_format, output_register, entity_id, comparison_type, health_comparison)

# 3
def if_character_human(output_register, entity_id):
    # Check if specified character entity is human ("Survival").
    return if_character_type(output_register, entity_id, 0)
def if_character_hollow(output_register, entity_id):
    # Check if specified character entity is hollow ("Gray Ghost").
    return if_character_type(output_register, entity_id, 8)
def if_character_type(output_register, entity_id, character_type):
    ...
    return _format_event(event_format, output_register, entity_id, character_type)

# 4
def if_entity_targeting(output_register, entity_id, targeted_entity_id):
    # Check if first entity is targeting second.
    return if_entity_target_state(output_register, entity_id, targeted_entity_id, 1)
def if_entity_not_targeting(output_register, entity_id, targeted_entity_id):
    # Check if first entity is not targeting second.
    return if_entity_target_state(output_register, entity_id, targeted_entity_id, 0)
def if_entity_target_state(output_register, entity_id, targeted_entity_id, required_target_state):
    ...
    return _format_event(event_format, output_register, entity_id, targeted_entity_id, required_target_state)

# 5
def if_entity_has_special_effect(output_register, entity_id, special_effect_id):
    # Check if entity has specified special effect.
    return if_entity_special_effect_state(output_register, entity_id, special_effect_id, 1)
def if_entity_does_not_have_special_effect(output_register, entity_id, special_effect_id):
    # Check if entity does not have specified special effect.
    return if_entity_special_effect_state(output_register, entity_id, special_effect_id, 0)
def if_entity_special_effect_state(output_register, entity_id, special_effect_id, required_state):
    # Check if entity has (1) or doesn't have (0) specified special effect.
    event_format = ['   4', '05', 'biiB']
    return _format_event(event_format, output_register, entity_id, special_effect_id, required_state)
    
# 6
def if_NPC_part_health_less_than_or_equal(output_register, entity_id, part_NPC_type, health_threshold):
    return if_NPC_part_health_comparison(output_register, entity_id, part_NPC_type, health_threshold, 5)
def if_NPC_part_health_comparison(output_register, entity_id, part_NPC_type, health_threshold, comparison_type):
    # Check comparison of NPC health part. I only really plan on copying this.
    event_format = ['   4', '06', 'biiib']
    return _format_event(event_format, output_register, entity_id, part_NPC_type, health_threshold, comparison_type)

# 7
def if_entity_background_loaded(output_register, entity_id):
    return if_entity_background_load_status(output_register, entity_id, 1)
def if_entity_not_background_loaded(output_register, entity_id):
    return if_entity_background_load_status(output_register, entity_id, 0)
def if_entity_background_load_status(output_register, entity_id, loaded):
    # Check if entity is loaded in background (presumably) or not.
    event_format = ['   4', '07', 'biB']
    return _format_event(event_format, output_register, entity_id, loaded)
    
# 8
def if_event_message_ID_match(output_register, entity_id, event_message_id):
    return if_event_message_ID_match_state(output_register, entity_id, event_message_id, 1)
def if_event_message_ID_does_not_match(output_register, entity_id, event_message_id):
    return if_event_message_ID_match_state(output_register, entity_id, event_message_id, 0)
def if_event_message_ID_match_state(output_register, entity_id, event_message_id, match_state):
    ...
    return _format_event(event_format, output_register, entity_id, event_message_id, match_state)

# 9
def if_ai_state(output_register, entity_id, required_ai_state):
    ...
    return _format_event(event_format, output_register, entity_id, required_ai_state)

# 10
def if_skull_lantern_activated(output_register):
    ...
    return _format_event(event_format, output_register, 1)
def if_skull_lantern_not_activated(output_register):
    # Check if player is using (holding out) the Skull Lantern. Currently used
    # to alter enemy aggression in Tomb of the Giants, I think. No need for a
    # generic version.
    event_format = ['   4', '10', 'bB']
...
# 11
def if_player_class(output_register, class_name):
    ...
    return _format_event(event_format, output_register, class_name)
    
# 12
def if_player_covenant(output_register, covenant_name):
    ...
    if isinstance(covenant_name, str):
        if covenant_name.lower() in covenant_list:
            ...
            raise ValueError('Unrecognized class name.')
    event_format = ['   4', '11', 'bB']
    return _format_event(event_format, output_register, covenant_name)

# 13
def if_player_soul_level_greater_than_or_equal(output_register, comparison_value):
    return if_player_soul_level_comparison(output_register, 4, comparison_value)
def if_player_soul_level_less_than_or_equal(output_register, comparison_value):
    return if_player_soul_level_comparison(output_register, 5, comparison_value)
def if_player_soul_level_comparison(output_register, comparison_type, comparison_value):
    # Check if player soul level comparison returns true.
    event_format = ['   4', '13', 'bBI']
    return _format_event(event_format, output_register, comparison_type, comparison_value)

# 14
def if_entity_health_value_equal(output_register, entity_id, comparison_value):
    return if_entity_health_value_comparison(output_register, entity_id, 0, comparison_value)
def if_entity_health_value_not_equal(output_register, entity_id, comparison_value):
    return if_entity_health_value_comparison(output_register, entity_id, 1, comparison_value)
def if_entity_health_value_greater_than(output_register, entity_id, comparison_value):
    return if_entity_health_value_comparison(output_register, entity_id, 2, comparison_value)
def if_entity_health_value_less_than(output_register, entity_id, comparison_value):
    return if_entity_health_value_comparison(output_register, entity_id, 3, comparison_value)
def if_entity_health_value_greater_than_or_equal(output_register, entity_id, comparison_value):
    return if_entity_health_value_comparison(output_register, entity_id, 4, comparison_value)
def if_entity_health_value_less_than_or_equal(output_register, entity_id, comparison_value):
    return if_entity_health_value_comparison(output_register, entity_id, 5, comparison_value)
def if_entity_health_value_comparison(output_register, entity_id, comparison_type, comparison_value):
    # Check if absolute entity health (NOT ratio) comparison returns true.
    event_format = ['   4', '14', 'biBi']
    return _format_event(event_format, output_register, entity_id, comparison_type, comparison_value)

""" 5: CONDITION REGISTERS (OBJECT) """

# 0
def if_object_destroyed(output_register, entity_id):
    return if_object_destruction_state(output_register, 1, entity_id)
def if_object_not_destroyed(output_register, entity_id):
    return if_object_destruction_state(output_register, 0, entity_id)
def if_object_destruction_state(output_register, required_state, entity_id):
    # Check if object is destroyed or not.
    event_format = ['   5', '00', 'bBi']
    return _format_event(event_format, output_register, required_state, entity_id)

# 1
def if_entity_damaged_object(output_register, entity_id, attacker_entity_id):
    # Check if object was damaged by a specific attacker.
    event_format = ['   5', '01', 'bii']
    return _format_event(event_format, output_register, entity_id, attacker_entity_id)

# 2
def if_object_activated(output_register, execution_event_id):
    # Check if object was activated.
    event_format = ['   5', '02', 'bi']
    return _format_event(event_format, output_register, execution_event_id)

""" 11: CONDITION REGISTERS (HIT) """

# 0
def if_player_moving_on_hitbox(output_register, hitbox_entity_id):
    # Check if a local player is moving on the specified hitbox.
    event_format = ['   5', '00', 'bi']
    return _format_event(event_format, output_register, hitbox_entity_id)

# 1
def if_player_running_on_hitbox(output_register, hitbox_entity_id):
    # Check if a local player is running on the specified hitbox.
    event_format = ['   5', '01', 'bi']
    return _format_event(event_format, output_register, hitbox_entity_id)

# 2
def if_player_standing_on_hitbox(output_register, hitbox_entity_id):
